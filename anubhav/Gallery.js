...
const SmoothCoverflowCarousel = () => {
  ...
  const getSlideStyle = (position) => {
    ...
    const positionTransforms = {
      '-2': `${baseTransform} translateX(${-2 * xUnit}px) translateZ(${2 * zUnit}px) scale(${isMobile ? 0.6 : 0.5}) rotateY(30deg)`,
      '-1': `${baseTransform} translateX(${-1 * xUnit}px) translateZ(${1 * zUnit}px) scale(${isMobile ? 0.8 : 0.75}) rotateY(15deg)`,
