...
const TopicBox = ({ title, desc, left }) => (
  <div className="relative group hover:z-50 hover:scale-[1.05] hover:shadow-xl transition duration-300">
    <div className="cursor-pointer px-4 py-2 rounded-lg shadow-md text-white text-sm font-semibold text-center 
    ...
    <div className={`absolute z-50 left-1/2 -translate-x-1/2 top-full mt-3 w-64 p-4 
      ${left ? 'hover-animated-gradient' : 'bg-gradient-to-br from-gray-700 via-gray-600 to-gray-800'} 
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-purple-500 via-pink-500 to-red-400 flex items-center justify-center">
      <div className="bg-white shadow-2xl rounded-3xl p-6 w-284 pt-16 pb-16 max-w-6xl w-full">
        <h1 className="text-3xl font-bold text-center mb-6 text-gray-800">SYLLABUS AND LEARNING</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center relative z-10 overflow-visible">
          ...
          <div className="space-y-4 relative z-10">
            {leftTopics.map((topic, index) => (
              <TopicBox key={index} title={topic.title} desc={topic.desc} left />
            ))}
          </div>

          {/* Center Robot */}
          <div id="robot-img" class="flex justify-center items-center transform transition duration-300 hover:scale-105">
  <img
  src={front_section}
  alt="Shiksha Robot"
